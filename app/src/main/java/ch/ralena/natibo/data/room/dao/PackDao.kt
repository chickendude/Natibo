...
interface PackDao {
	...
	@Transaction
	@Query("SELECT * FROM packroom WHERE name = :name AND languageId IN (:languageIds)")
	suspend fun getWithSentencesByNameAndLanguage(name: String, languageIds: List<Long>): PackWithSentences?
